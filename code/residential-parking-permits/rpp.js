...
$(document).on('knack-view-render.view_237', function(event, page) {
  // create large button on the home page
    bigButton('rpp', 'view_237', "https://atd.knack.com/parking#rpp", "home", "Home");
});

$(document).on('knack-view-render.view_127', function(event, page) {
  // create large button on the home page
    bigButton('apply', 'view_127', "https://atd.knack.com/parking#apply/", "arrow-right", "Start Application");
});
...
$(document).on('knack-form-submit.view_163', function(event, view, record) {
  ...
  point_in_poly('RPP_Properties',0, lat, lon, street, "*", function(feature, lat, lon, address) {
    ...
    if (feature) {
      ...
      var message = '<h5><i class="fa fa-exclamation-triangle"></i> Our records show that <underline>no parking permit is required</underline> at this address. If you think this may be in error please email: <b>residentialparking@austintexas.gov</b></h5>';
...
function point_in_poly(service_name, layer_id, lat, lon, address, outfields, callback) {
...
function geocode(address) {
  ...
  var url = 'https://www.austintexas.gov/GIS/REST/Geocode/COA_Street_Locator/GeocodeServer/findAddressCandidates?SingleLine=' + address
...
function getIframe(el_id, street) {
  var url = 'https://www.google.com/maps/embed/v1/place?key=AIzaSyBhvDuAf3jPNzdnowYI4beNjEK4lkWJiKI&zoom=18&q=' + street + ',austin,tx';
  console.log(url);
  var width = $('#knack-body').width();
  var width = width * .9;
  //  set iframe source
  $( '#' + el_id ).attr('src', url).attr('width', width); 
  //  reveal iframe
  $('#' + el_id).show();
...
function addButton(button_id, element, href, icon, label) {
  ...
  var button = '<a class="big-button" href="' + href + '"><div class="big-button-container"><span><i class="fa fa-' + icon + '" ></i></span><span> ' + label + '</span></div></a>'
...
function showMessage(element, message='', message_type='warning') {
  ...
  $('<div class="kn-view"><div class="kn-notification is-' + message_type + '">' + message + '</div></div>').prependTo(element);
  return true;
}
...
function getMapUrl(lat, lon, address) {
  address = encodeURI(address);
  return 'http://austin.maps.arcgis.com/apps/webappviewer/index.html?id=27116dbd37274d298315b3c9fdb4319a&level=10&marker=' + lon + ',' + lat + ',4632,,,' + address;
}
